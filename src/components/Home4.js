...
const FeatureCarousel = () => {
  ...
  return (
    <div className="w-full rounded-[3rem] mt-44 padding-reset bg-[#13182f] mb-7 h-[49rem] flex flex-col relative sm:flex-row overflow-hidden items-center sm:items-center justify-center">
      
      {/* Left Side (keep this section the same) */}
      <div className="w-full sm:w-[30vw] px-6 sm:p-12 flex flex-col items-start justify-center text-white">
        <p className="text-blue-600 uppercase font-semibold tracking-widest">Specialities</p>
        <h2 className="text-3xl sm:text-5xl font-bold mt-3 leading-normal">Our wide range of specialities</h2>
        <p className="text-gray-300 mt-6">
        Lorem ipsum dolor sit amet consectetur non venenatis nisi et in quis fermentum vel magna.
        </p>
         <button className="bg-[#ffffff] mt-5 flex flex-row gap-1 items-center border border-black text-xl font-normal text-gray-800 py-6 px-7 rounded-full">
                   <Image
                     src="/Icons/star.svg"
                     alt="Twitter"
                     width={24}
 ...
 {/* Left Arrow */}
        <button 
          className="absolute left-0 z-50 bg-blue-700 text-gray-200 p-5 rounded-full shadow-lg hover:bg-blue-500"
          onClick={prevSlide}
        >
          <FaArrowLeft size={32} />
        </button>
        {/* Slide Container */}
        <div className="overflow-hidden w-[70%] sm:w-[60vw] h-auto">
          <div
          ...
          >
            {slides.map((slide, index) => (
              <div 
                key={slide.id}
                className="flex-shrink-0 h-[25rem] mx-2 transition-all duration-300"
                style={{ 
                  width: `${cardWidth}vw`,
              ...
              >
                <div className="w-full h-full bg-white  rounded-3xl flex flex-col items-center text-center p-6">
                  ...
                  <p className="text-gray-600 mt-4   px-4 w-80">{slide.description}</p>
        ...
        {/* Right Arrow */}
        <button 
          className="absolute right-0 sm:right-10  bg-blue-700 text-gray-200 p-5 rounded-full shadow-lg hover:bg-blue-500 z-40"
          onClick={nextSlide}
        >
